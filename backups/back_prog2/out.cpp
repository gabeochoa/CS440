...
struct Deque_MyClass_Iterator { MyClass *d_data; int d_ind; void inc(Deque_MyClass_Iterator* it){it->d_ind++;} MyClass& deref(Deque_MyClass_Iterator* it){return it->d_data[it->d_ind];} void dec(Deque_MyClass_Iterator* it){it->d_ind--;} }; struct Deque_MyClass { MyClass *d_data; int d_size; int d_alloc; int d_front; int d_back; typedef bool (*comp)(const MyClass &a, const MyClass &b); comp func; char type_name[6 + sizeof("MyClass")] = "Deque_"; Deque_MyClass_Iterator *d_dib; Deque_MyClass_Iterator *d_die; void push_back(Deque_MyClass *d, MyClass data); void push_front(Deque_MyClass *d, MyClass data); void pop_back(Deque_MyClass *d); void pop_front(Deque_MyClass *d); MyClass& front(Deque_MyClass *d); MyClass& back(Deque_MyClass *d); Deque_MyClass_Iterator& begin(Deque_MyClass *d); Deque_MyClass_Iterator& end(Deque_MyClass *d); MyClass& at(Deque_MyClass *d, int pos); int size(Deque_MyClass *d); bool empty(Deque_MyClass *d); void dtor(Deque_MyClass *d); void clear(Deque_MyClass *d); void checkreset(Deque_MyClass *d); }; static void Deque_MyClass_ctor(Deque_MyClass *d, bool (*f)(const MyClass &a, const MyClass &b)) { d->d_data = ( MyClass *) malloc(sizeof( MyClass )*10); d->d_size = 0; d->d_alloc = 10; d->d_front = 0; d->d_back = 0; d->d_dib = new Deque_MyClass_Iterator(); d->d_die = new Deque_MyClass_Iterator(); d->func = f; strcat(d->type_name, "MyClass"); } static void Deque_MyClass_Iterator_ctor(Deque_MyClass_Iterator* it, Deque_MyClass *d, bool front) { it->d_data = d->d_data; if(front) { it->d_ind = d->d_front; } else { it->d_ind = d->d_back; } } static bool Deque_MyClass_Iterator_equal(const Deque_MyClass_Iterator& iter, const Deque_MyClass_Iterator& iter2) { return iter.d_ind == iter2.d_ind; } static bool Deque_MyClass_equal( Deque_MyClass &iter, Deque_MyClass &iter2) { bool ret = true; if(iter.d_size != iter2.d_size) { return false; } for(int i=0; i< iter.d_size; i++) { MyClass val = (iter.d_data[i]); MyClass val2 = (iter2.d_data[i]); bool alessb = iter.func(val, val2); bool blessa = iter.func(val2, val); if( !alessb && !blessa ) { } else { ret = false; break; } } return ret; } void Deque_MyClass::push_back(Deque_MyClass *d, MyClass data) { if(d->d_alloc == d->d_size) { MyClass *temp = ( MyClass *) malloc(sizeof( MyClass )*d->d_alloc*2); std::cout << d->d_alloc << " points" << std::endl; int c=0; for(int i=d->d_front; i<d->d_back; i++) { temp[c] = d->d_data[i]; c++; } free(d->d_data); d->d_data = temp; d->d_alloc *= 2; d->d_front = 0; d->d_back = d->d_size+1; } d->d_data[d->d_back++] = data; d->d_size++; } void Deque_MyClass::push_front(Deque_MyClass *d, MyClass data) { if(d->d_front == 0) { if(d->d_alloc == d->d_size) { MyClass *temp = (MyClass *) malloc(sizeof( MyClass )*d->d_alloc*2); int c=1; for(int i=d->d_front; i<=d->d_back; i++) { temp[c] = d->d_data[i]; c++; } free(d->d_data); d->d_data = temp; d->d_alloc *= 2; d->d_data[0] = data; d->d_back = d->d_size+1; d->d_front = 0; } else { for(int i=d->d_back; i>=d_front; i--) { d->d_data[i] = d->d_data[i-1]; } d->d_data[d_front] = data; d->d_back+=1; } } else { d->d_data[--d_front] = data; } d->d_size++; } void Deque_MyClass::pop_back(Deque_MyClass *d) { if(d->d_back == 0) { } else { d->d_back--; d->d_size--; } checkreset(d); } void Deque_MyClass::pop_front(Deque_MyClass *d) { if(d->d_front == d->d_back) { } else { d->d_front++; d->d_size--; } checkreset(d); } void Deque_MyClass::checkreset(Deque_MyClass *d) { if(d->d_size < d->d_alloc/4) { MyClass *temp = (MyClass *) malloc(sizeof( MyClass )*d->d_alloc/4); int c=1; for(int i=d->d_front; i<=d->d_back; i++) { temp[c] = d->d_data[i]; c++; } free(d->d_data); d->d_data = temp; d->d_alloc /= 4; std::cout << "SMALLER ALLOC"<<d->d_alloc << std::endl; } } MyClass& Deque_MyClass::front(Deque_MyClass *d) { return d->d_data[d_front]; } MyClass& Deque_MyClass::back(Deque_MyClass *d) { return d->d_data[d->d_back-1]; } Deque_MyClass_Iterator& Deque_MyClass::begin(Deque_MyClass *d) { if(d->d_dib) free(d->d_dib); d_dib = new Deque_MyClass_Iterator(); Deque_MyClass_Iterator_ctor(d->d_dib, d, true); return *d_dib; } Deque_MyClass_Iterator& Deque_MyClass::end(Deque_MyClass *d) { if(d->d_die) free(d->d_die); d_die = new Deque_MyClass_Iterator(); Deque_MyClass_Iterator_ctor(d->d_die, d, false); return *d_die; } MyClass& Deque_MyClass::at(Deque_MyClass *d, int pos) { return d->d_data[d->d_front+pos]; } int Deque_MyClass::size(Deque_MyClass *d) { return d->d_size; } bool Deque_MyClass::empty(Deque_MyClass *d) { return d->d_size==0; } void Deque_MyClass::dtor(Deque_MyClass *d) { if(d->d_die) free(d->d_die); if(d->d_dib) free(d->d_dib); free(d->d_data); } void Deque_MyClass::clear(Deque_MyClass *d) { d->d_front = 0; d->d_back = 0; d->d_size = 0; }
...
struct Deque_int_Iterator { int *d_data; int d_ind; void inc(Deque_int_Iterator* it){it->d_ind++;} int& deref(Deque_int_Iterator* it){return it->d_data[it->d_ind];} void dec(Deque_int_Iterator* it){it->d_ind--;} }; struct Deque_int { int *d_data; int d_size; int d_alloc; int d_front; int d_back; typedef bool (*comp)(const int &a, const int &b); comp func; char type_name[6 + sizeof("int")] = "Deque_"; Deque_int_Iterator *d_dib; Deque_int_Iterator *d_die; void push_back(Deque_int *d, int data); void push_front(Deque_int *d, int data); void pop_back(Deque_int *d); void pop_front(Deque_int *d); int& front(Deque_int *d); int& back(Deque_int *d); Deque_int_Iterator& begin(Deque_int *d); Deque_int_Iterator& end(Deque_int *d); int& at(Deque_int *d, int pos); int size(Deque_int *d); bool empty(Deque_int *d); void dtor(Deque_int *d); void clear(Deque_int *d); void checkreset(Deque_int *d); }; static void Deque_int_ctor(Deque_int *d, bool (*f)(const int &a, const int &b)) { d->d_data = ( int *) malloc(sizeof( int )*10); d->d_size = 0; d->d_alloc = 10; d->d_front = 0; d->d_back = 0; d->d_dib = new Deque_int_Iterator(); d->d_die = new Deque_int_Iterator(); d->func = f; strcat(d->type_name, "int"); } static void Deque_int_Iterator_ctor(Deque_int_Iterator* it, Deque_int *d, bool front) { it->d_data = d->d_data; if(front) { it->d_ind = d->d_front; } else { it->d_ind = d->d_back; } } static bool Deque_int_Iterator_equal(const Deque_int_Iterator& iter, const Deque_int_Iterator& iter2) { return iter.d_ind == iter2.d_ind; } static bool Deque_int_equal( Deque_int &iter, Deque_int &iter2) { bool ret = true; if(iter.d_size != iter2.d_size) { return false; } for(int i=0; i< iter.d_size; i++) { int val = (iter.d_data[i]); int val2 = (iter2.d_data[i]); bool alessb = iter.func(val, val2); bool blessa = iter.func(val2, val); if( !alessb && !blessa ) { } else { ret = false; break; } } return ret; } void Deque_int::push_back(Deque_int *d, int data) { if(d->d_alloc == d->d_size) { int *temp = ( int *) malloc(sizeof( int )*d->d_alloc*2); std::cout << d->d_alloc << " points" << std::endl; int c=0; for(int i=d->d_front; i<d->d_back; i++) { temp[c] = d->d_data[i]; c++; } free(d->d_data); d->d_data = temp; d->d_alloc *= 2; d->d_front = 0; d->d_back = d->d_size+1; } d->d_data[d->d_back++] = data; d->d_size++; } void Deque_int::push_front(Deque_int *d, int data) { if(d->d_front == 0) { if(d->d_alloc == d->d_size) { int *temp = (int *) malloc(sizeof( int )*d->d_alloc*2); int c=1; for(int i=d->d_front; i<=d->d_back; i++) { temp[c] = d->d_data[i]; c++; } free(d->d_data); d->d_data = temp; d->d_alloc *= 2; d->d_data[0] = data; d->d_back = d->d_size+1; d->d_front = 0; } else { for(int i=d->d_back; i>=d_front; i--) { d->d_data[i] = d->d_data[i-1]; } d->d_data[d_front] = data; d->d_back+=1; } } else { d->d_data[--d_front] = data; } d->d_size++; } void Deque_int::pop_back(Deque_int *d) { if(d->d_back == 0) { } else { d->d_back--; d->d_size--; } checkreset(d); } void Deque_int::pop_front(Deque_int *d) { if(d->d_front == d->d_back) { } else { d->d_front++; d->d_size--; } checkreset(d); } void Deque_int::checkreset(Deque_int *d) { if(d->d_size < d->d_alloc/4) { int *temp = (int *) malloc(sizeof( int )*d->d_alloc/4); int c=1; for(int i=d->d_front; i<=d->d_back; i++) { temp[c] = d->d_data[i]; c++; } free(d->d_data); d->d_data = temp; d->d_alloc /= 4; std::cout << "SMALLER ALLOC"<<d->d_alloc << std::endl; } } int& Deque_int::front(Deque_int *d) { return d->d_data[d_front]; } int& Deque_int::back(Deque_int *d) { return d->d_data[d->d_back-1]; } Deque_int_Iterator& Deque_int::begin(Deque_int *d) { if(d->d_dib) free(d->d_dib); d_dib = new Deque_int_Iterator(); Deque_int_Iterator_ctor(d->d_dib, d, true); return *d_dib; } Deque_int_Iterator& Deque_int::end(Deque_int *d) { if(d->d_die) free(d->d_die); d_die = new Deque_int_Iterator(); Deque_int_Iterator_ctor(d->d_die, d, false); return *d_die; } int& Deque_int::at(Deque_int *d, int pos) { return d->d_data[d->d_front+pos]; } int Deque_int::size(Deque_int *d) { return d->d_size; } bool Deque_int::empty(Deque_int *d) { return d->d_size==0; } void Deque_int::dtor(Deque_int *d) { if(d->d_die) free(d->d_die); if(d->d_dib) free(d->d_dib); free(d->d_data); } void Deque_int::clear(Deque_int *d) { d->d_front = 0; d->d_back = 0; d->d_size = 0; }
...
main() {
    {
...

        {
            ...
            deq1.pop_back(&deq1);
            ((!Deque_MyClass_equal(deq1, deq2)) ? static_cast<void> (0) : __assert_fail ("!Deque_MyClass_equal(deq1, deq2)", "test.cpp", 215, __PRETTY_FUNCTION__));
            deq1.push_back(&deq1, MyClass{4, "Mary"});
            ((!Deque_MyClass_equal(deq1, deq2)) ? static_cast<void> (0) : __assert_fail ("!Deque_MyClass_equal(deq1, deq2)", "test.cpp", 217, __PRETTY_FUNCTION__));
...

    {
        ...
        ((deq.size(&deq) == 0) ? static_cast<void> (0) : __assert_fail ("deq.size(&deq) == 0", "test.cpp", 229, __PRETTY_FUNCTION__));
        ((deq.empty(&deq)) ? static_cast<void> (0) : __assert_fail ("deq.empty(&deq)", "test.cpp", 230, __PRETTY_FUNCTION__));


        printf("---- %s, %d\n", deq.type_name, int(sizeof(deq.type_name)));

        ((sizeof deq.type_name == 10) ? static_cast<void> (0) : __assert_fail ("sizeof deq.type_name == 10", "test.cpp", 235, __PRETTY_FUNCTION__));
        ...
        printf("%d\n", deq.front(&deq));
        printf("%d\n", deq.back(&deq));
        ((deq.front(&deq) == -1) ? static_cast<void> (0) : __assert_fail ("deq.front(&deq) == -1", "test.cpp", 245, __PRETTY_FUNCTION__));
        ((deq.back(&deq) == 3) ? static_cast<void> (0) : __assert_fail ("deq.back(&deq) == 3", "test.cpp", 246, __PRETTY_FUNCTION__));

        deq.pop_front(&deq);
        deq.pop_back(&deq);
        ((deq.front(&deq) == 0) ? static_cast<void> (0) : __assert_fail ("deq.front(&deq) == 0", "test.cpp", 250, __PRETTY_FUNCTION__));
        ((deq.back(&deq) == 2) ? static_cast<void> (0) : __assert_fail ("deq.back(&deq) == 2", "test.cpp", 251, __PRETTY_FUNCTION__));

        ((deq.size(&deq) == 3) ? static_cast<void> (0) : __assert_fail ("deq.size(&deq) == 3", "test.cpp", 253, __PRETTY_FUNCTION__));
...

        {
            auto it = deq.end(&deq);
            it.dec(&it);
            ((it.deref(&it) == 2) ? static_cast<void> (0) : __assert_fail ("it.deref(&it) == 2", "test.cpp", 264, __PRETTY_FUNCTION__));
        }
...

    {
        ...
        deq1.pop_back(&deq1);
        ((!Deque_int_equal(deq1, deq2)) ? static_cast<void> (0) : __assert_fail ("!Deque_int_equal(deq1, deq2)", "test.cpp", 295, __PRETTY_FUNCTION__));
        deq1.push_back(&deq1, 4);
        ((!Deque_int_equal(deq1, deq2)) ? static_cast<void> (0) : __assert_fail ("!Deque_int_equal(deq1, deq2)", "test.cpp", 297, __PRETTY_FUNCTION__));
